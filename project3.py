...
url_user_recom_reuslt = 'https://drive.google.com/file/d/1O3_f8hq0kzyXNC1bJfPBHWzWJpdBpwfK/view?usp=sharing'
...
if choice == '1. Khám phá dữ liệu':
    ...
    # read data
    user_recommendation_result = read_file_from_ggdr(url_user_recom_reuslt)
    data_cleaned = read_file_from_ggdr(url_data_cleand)
    df_product = data_cleaned[['item_id', 'name', 'rating', 'price', 'brand', 'image', 'group1']]
    ...
    # choose bar
    st.sidebar.markdown('***Chọn thông tin***')
    customer_id = st.sidebar.selectbox('ID người dùng', user_recommendation_result['customer_id'])
    lst_num = [2, 3, 4, 5, 6, 7, 8, 9, 10]
    items_num = st.sidebar.selectbox('Số sản phẩm muốn đề xuất', lst_num)
    submit_button = st.sidebar.button(label='Summit')
